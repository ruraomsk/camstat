...
func (d *DeviceModbus) makeNewMap() {
	d.Intervals = make(map[int]OneSecondData)
	var value [16]int
...
func (d *DeviceModbus) sendStatistics(ptime int) {
	if !d.isInterval {
		return
	}
	var value [16]int
...
func (d *DeviceModbus) addData(writeTime int) {
	if !d.isInterval {
		return
	}
	t := writeTime % d.Tsum
...
func (d *DeviceModbus) getValue() [16]int {
	var res [16]int
	base := 0
	if d.Type == 1 {
		for i := 0; i < 16; i++ {
